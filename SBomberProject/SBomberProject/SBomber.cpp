...
void SBomber::CheckBombsAndGround()
{
    ...
    DestroyableGroundObject* pDestroyableGroundObject;
    ...
    for (size_t i = 0; i < vecBombs.size(); i++)
    {
        if (vecBombs[i]->GetY() >= y) // Пересечение бомбы с землей
        {
            ...
            vecBombs[i]->AddObserver(vecDestoyableObjects);
            pDestroyableGroundObject = vecBombs[i]->CheckDestoyableObjects();
            if (pDestroyableGroundObject != nullptr)
            {
                score += pDestroyableGroundObject->GetScore();
                DeleteStaticObj(pDestroyableGroundObject);
            }
